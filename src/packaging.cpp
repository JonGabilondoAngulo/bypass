           ...
           boost::filesystem::path & outRepackedIPAPath)
{
    ...
    boost::filesystem::path repackedAppPath( boost::filesystem::temp_directory_path() / "hpeRepackedIpa.ipa");
                                ...
                                boost::filesystem::path & outNewIPAPath)
{
    ...
    if (!newIPAName.empty()) {
        ...
        if (wasInjected) {
            if (newAppName.find("+bypass") == std::string::npos) {
                newAppName = originalIPAfilename.replace_extension("").string();
                newAppName += "+bypass.ipa";
            }
        } else {
